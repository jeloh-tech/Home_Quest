            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
                <div class="max-w-7xl mx-auto px-6 py-4">
                        <div class="flex items-center justify-between">
                            <a href="{{ route('landlord.messages') }}" class="flex items-center text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-140px)]">
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 h-full">
                            ...
                            <div class="space-y-3">
                                <a href="{{ route('landlord.messages') }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors group">
                            ...
                            <div class="p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 rounded-t-2xl">
                                <div class="flex items-center space-x-4">
                                    @if($otherUser->profile_photo_path)
                                        <img src="{{ asset('storage/' . $otherUser->profile_photo_path) }}" alt="{{ $otherUser->name }}" class="w-14 h-14 rounded-full object-cover ring-4 ring-white dark:ring-gray-800">
                                    @else
                                        <div class="w-14 h-14 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center ring-4 ring-white dark:ring-gray-800">
                                            <span class="text-gray-600 dark:text-gray-400 font-semibold text-xl">{{ substr($otherUser->name, 0, 1) }}</span>
                                        </div>
                                    @endif
                                    <div>
                                ...
                                @forelse($messages as $message)
                                    ...
                                    @else
                                        ...
                                        <div class="flex items-start space-x-3">
                                            @if($otherUser->profile_photo_path)
                                                <img src="{{ asset('storage/' . $otherUser->profile_photo_path) }}" alt="{{ $otherUser->name }}" class="w-8 h-8 rounded-full object-cover">
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center">
                                                    <span class="text-gray-600 dark:text-gray-400 font-semibold text-xs">{{ substr($otherUser->name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                            <div class="flex-1">
...
let currentUserId = {!! json_encode(auth()->id()) !!};
let otherUserId = {!! json_encode($otherUser->id) !!};
let currentUserInitial = {!! json_encode(substr(auth()->user()->name, 0, 1)) !!};
let otherUserAvatar = {!! json_encode($otherUser->profile_photo_path ? asset('storage/' . $otherUser->profile_photo_path) : '') !!};
let otherUserName = {!! json_encode($otherUser->name) !!};
let otherUserInitial = {!! json_encode(strtoupper(substr($otherUser->name, 0, 1))) !!};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (messageForm) {
        messageForm.addEventListener('submit', function(e) {
            ...
            fetch(`{{ route('landlord.messages.send', $otherUser->id) }}`, {
                method: 'POST',
                headers: {
...
function addMessageToUI(message, isSender) {
    ...
    if (isSender) {
        ...
        if (otherUserAvatar) {
            ...
            avatarDiv.className = 'w-8 h-8 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center';
            avatarDiv.innerHTML = `<span class="text-gray-600 dark:text-gray-400 font-semibold text-xs">${otherUserInitial}</span>`;
        }
    }
